...
from vec2.config import settings
...
class PolarTaskError(Vec2Error):
    """
...
class PolarRedirectionError(Vec2Error):
    """
...
class InternalServerError(Vec2Error):
    def __init__(
...
class ResourceNotFound(Vec2Error):
...
class ResourceUnavailable(Vec2Error):
    def __init__(self, message: str = "Unavailable", status_code: int = 410) -> None:
        super().__init__(message, status_code)


class ResourceAlreadyExists(Vec2Error):
...
class PolarRequestValidationError(Vec2Error):
